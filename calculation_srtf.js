...
function displayResultTable(processes, awt, atat, ganttChart) {
    ...
    let output = '<table>';
    output += '<tr><th>Process</th><th>Arrival Time</th><th>Burst Time</th><th>Completion Time</th><th>Waiting Time</th><th>Turnaround Time</th></tr>';

    for (let i = 0; i < processes.length; i++) {
        const completionTime = processes[i].at + processes[i].wt + processes[i].bt;
        output += `<tr><td>P${i + 1}</td><td>${processes[i].at}</td><td>${processes[i].bt}</td><td>${completionTime}</td><td>${processes[i].wt}</td><td>${completionTime - processes[i].at}</td></tr>`;
    }
